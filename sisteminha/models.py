...
class Microempreendedor(models.Model):
    ...
    foto = models.ImageField(
        upload_to='imgs/microempreendedores', default='imgs/avatar.png')
    user = models.OneToOneField(User, on_delete=models.CASCADE,
                                blank=True, null=True, related_name='microempreendedor')
...
class Desenvolvedor(models.Model):
    ...
    foto = models.ImageField(
        upload_to='imgs/desenvolvedores', default='imgs/avatar.png')
    descricao = models.TextField(default="Descrição está vazia.")
    github = models.CharField(max_length=20)
    user = models.OneToOneField(
        User, on_delete=models.CASCADE, blank=True, null=True, related_name='desenvolvedor')

    def __str__(self):
        # Retorna o nome do usuário vinculado
...
class Avaliacao_Sistema(models.Model):
    ...
    estrela = models.IntegerField(choices=ESTRELA_CHOICES, validators=[
                                  MinValueValidator(1), MaxValueValidator(5)], null=False)
...
class Avaliacao_Desenvolvedor(models.Model):
    ...
    desenvolvedor = models.ForeignKey(
        Desenvolvedor, related_name="avaliacoes", on_delete=models.CASCADE)
