...
def test_avaliacao_desenvolvedor_api(authenticated_admin_client, admin_user):
    desenvolvedor = Desenvolvedor.objects.create(
        user=admin_user, cpf="12345678901", github="devhub"
    )
...
def test_avaliacao_desenvolvedor_str_method(admin_user):
    ...
    expected_str = (
        f"Avaliação de {admin_user.username} para {desenvolvedor.user.username}"
    )
